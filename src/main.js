...
function readControlOffsets() {
  try {
    ...
    if (parsed && typeof parsed === 'object') controlOffsets = parsed;
  } catch (error) {
    controlOffsets = {};
  }
}
...
function applyControlOffsets() {
  for (const element of getAdjustableControls()) {
    ...
    element.style.setProperty('--drag-x', `${offset.x || 0}px`);
    element.style.setProperty('--drag-y', `${offset.y || 0}px`);
    element.style.setProperty('--control-scale', `${offset.scale || 1}`);
...
function setLayoutEditMode(enabled) {
  ...
  settingsModeText.textContent = enabled ? '正在调整：拖动按键；点选按键后可在设置里放大/缩小；点空白自动保存。' : '开启后可拖动虚拟按键，关闭后恢复游戏操作。';
  if (!enabled) {
...
function bindDraggableControl(element) {
  ...
  const moveDrag = (event, point, pointerId = null) => {
    ...
    const nextX = isRotatedLandscapeFallback() ? drag.baseX + dy : drag.baseX + dx;
    const nextY = isRotatedLandscapeFallback() ? drag.baseY - dx : drag.baseY + dy;
