...
const nativePostgameRom = postgameRom.slice();
nativePostgameRom.set(mapper198CompatibilityMarker, 8);
assert.equal(isKnownTunshiPostgameRom(nativePostgameRom), false, '独立卡带不能再安装网页专用剧情运行时');
assert.equal(isMmc3ChrRamExpansionRom(nativePostgameRom), true, '独立卡带只使用通用 M198 硬件协议');
...
const markedMapper198Rom = unrelated128BankRom.slice();
markedMapper198Rom.set(mapper198CompatibilityMarker, 8);
assert.equal(hasMapper198CompatibilityMarker(markedMapper198Rom), true);
assert.equal(isMmc3ChrRamExpansionRom(markedMapper198Rom), true, 'M198 硬件标记必须独立于游戏内容指纹');
const modifiedMarkedMapper198Rom = markedMapper198Rom.slice();
modifiedMarkedMapper198Rom[0x12345] ^= 0xff;
assert.equal(isMmc3ChrRamExpansionRom(modifiedMarkedMapper198Rom), true, '修改剧情数据后不应要求同步修改模拟器');
...
const protocolMapper = {
  ...
  write() {},
  toJSON: () => ({}),
  load8kRomBank(bank, address) { protocolLoads.push({ bank, address }); },
};
const protocolNes = { mmap: protocolMapper, cpu: { mem: new Uint8Array(0x10000) } };
assert.equal(installRomCompatibility(protocolNes, markedMapper198Rom), true);
protocolMapper.load8kRomBank(0x86, 0xa000);
assert.deepEqual(protocolLoads.at(-1), { bank: 0x06, address: 0xa000 });
protocolMapper.write(0x5ff0, 0x4d);
protocolMapper.write(0x5ff1, 0x98);
protocolMapper.load8kRomBank(0x86, 0xa000);
assert.deepEqual(protocolLoads.at(-1), { bank: 0x86, address: 0xa000 }, 'ROM 原生协议必须允许访问物理扩展 bank');
assert.equal(protocolMapper.toJSON().__mapper198PrgProtocol.extensionBanks, true, '扩展 bank 状态必须进入回滚快照');
protocolMapper.write(0x5ff0, 0x00);
protocolMapper.write(0x5ff1, 0x00);
protocolMapper.load8kRomBank(0x86, 0xa000);
assert.deepEqual(protocolLoads.at(-1), { bank: 0x06, address: 0xa000 }, '退出原生扩展后必须恢复 Mapper 198 别名');
...
assert.deepEqual(bankLoads, [], '兼容层不得改写 jsnes 已按 ROM 容量装入的物理固定 bank');
bankLoads.length = 0;
...
assert.deepEqual(bankLoads.slice(-2), [
  { bank: 0x7e, address: 0xc000 },
  { bank: 0x7f, address: 0xe000 },
], '退出扩展区时必须恢复物理固定 bank，不能套用 Mapper 198 别名掩码');
